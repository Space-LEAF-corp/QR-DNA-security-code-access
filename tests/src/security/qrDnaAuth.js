 ...
 */
export function verifySignature(publicKeyBase64, messageStr, signatureBase64) {
    try {
...
export class QrDnaAuth {
    ...
    static isAuthorized(request) {
        ...
        if (attestation) {
            const isValidSignature = verifySignature(attestation.publicKey, attestation.message, attestation.signature);
            if (!isValidSignature) {
                return false;
            }
